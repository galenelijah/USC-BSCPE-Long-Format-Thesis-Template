...
\begin{enumerate}
    ...
    \item \textbf{Generate Detailed Reports for Data Analysis and Decision-Making:} Implement a module for generating customizable reports on patient visits, clinical case distribution, and feedback analysis to aid decision-making.
...
\begin{enumerate}
    ...
    \begin{enumerate}
        \item Centralized and secure storage of all student health information, specifically medical histories (inclusive of patient concerns, vital signs, and diagnoses) and dental records (capturing oral health findings via the FDI notation system).
    ...
    \begin{enumerate}
        \item Integration of user-friendly digital evaluation forms to collect and analyze patient feedback. The user-friendliness of these forms was quantitatively measured during the User Acceptance Testing (UAT) phase using a 5.00-point Likert scale to assess intuitiveness and ease of navigation.
        \item Automated reminders dispatched both in-app and via email to encourage continuous patient input, enabling the clinic to identify areas for improvement.
    \end{enumerate}
    \item \textbf{Report Generation:}
    \begin{enumerate}
        \item The system will include a report generation module that allows clinic staff to create detailed, customizable reports on various metrics such as patient visits, clinical case distribution, and feedback analysis. These reports will be exportable to formats like PDF and Excel for further analysis and record-keeping.
    \end{enumerate}
    ...
    \begin{enumerate}
        \item The PIS will optimize the process of issuing medical certificates by allowing clinic staff to quickly access and input relevant patient information. The system generates a digital e-template for the USC Form ACA-HSD-04F, reducing administrative workload and speeding up the issuance process.
    \end{enumerate}
    \item \textbf{Platform:}
    \begin{enumerate}
        \item The system will be web-based, ensuring accessibility through standard web browsers, and is engineered to be fully mobile responsive to accommodate various device viewports.
    \end{enumerate}
    \item \textbf{Scalability:}
    \begin{enumerate}
        \item The system will be designed to scale for future iterations, including the potential for university-wide deployment and the integration of Progressive Web App (PWA) capabilities for offline access.
    \end{enumerate}
\end{enumerate}
