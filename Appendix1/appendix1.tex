...
The system strictly enforces user authorizations across hierarchical classes. Notably, all professional clinical roles (Admin, Doctor, Dentist, Nurse, and Staff) possess the authorization to create and export operational and clinical reports, while the System Audit Report is strictly reserved for the Admin role.
...
\hline
\textbf{Admin} & Possesses full system access, including global RBAC adjustments, system health monitoring, email system administration, and exclusive access to system audit reports. All clinical roles can create and export reports. \\
\hline
\textbf{Doctor / Dentist} & Endowed with full administrative and clinical access, only the Doctor role is uniquely authorized to approve medical certificates. Both can prescribe treatments, access all feedback analytics, and generate clinical reports. \\
\hline
\textbf{Nurse} & Supervisory and data-entry capacity; authorized to cater to patients, record vital signs, input treatment plans, generate clinical reports, and draft medical certificates for doctor approval. \\
\hline
\textbf{Staff} & Manages clinic administration, patient intake, certificate issuance, clinical report generation, and health campaign creation. All other clinical roles (Admin, Doctor, Dentist, and Nurse) possess the authorization to perform all tasks available to this Staff role. \\
\hline
...
\begin{itemize}
    ...
    \item \textbf{Clinical Records \& Retooling (patients):} Data entry uses a unified record component (\texttt{MedicalRecord.jsx} and \texttt{Dental.jsx}). \textbf{Retooling Focus:} An exhaustive "Application Retooling" phase integrated an advanced real-time filtering architecture (\texttt{year\_level} and \texttt{academic\_year}), the \textbf{Attributed Clinical Remarks} system for inter-professional coordination, and a completely overhauled professional reporting engine. This logic calculates Semesters autonomously to filter the patients list accurately.
...
\hline
\textbf{Search Latency} & Patient searches execute in \textbf{123.16ms--127.81ms} (Target: <500ms). & PT-04 \\
\hline
\textbf{PDF Generation} & Medical Certificate PDFs render in \textbf{122.93ms--124.53ms}. & PT-01 \\
\hline
